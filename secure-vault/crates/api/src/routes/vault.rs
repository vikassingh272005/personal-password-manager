...
pub async fn get_vault(
    ...
    let kdf_algorithm: String = row.try_get("kdf_algorithm").map_err(|_| AppError::NotFound)?;
    let kdf_memory: i32 = row.try_get("kdf_memory").map_err(|_| AppError::NotFound)?;
    let kdf_iterations: i32 = row.try_get("kdf_iterations").map_err(|_| AppError::NotFound)?;
    let kdf_parallelism: i32 = row.try_get("kdf_parallelism").map_err(|_| AppError::NotFound)?;
    ...
    let vault_updated_at: chrono::DateTime<chrono::Utc> = row
        .try_get("updated_at")
        .map_err(|_| AppError::NotFound)?;
    ...
    let b64 = base64::engine::general_purpose::STANDARD;
    let to_wrap = |ciphertext: Option<Vec<u8>>, nonce: Option<Vec<u8>>| {
    ...
    Ok(Json(VaultResponse {
        ...
        ciphertext: ciphertext
            .map(|ct| base64::Engine::encode(&b64, ct)),
...
pub async fn update_vault(
    ...
    let vault_exists: Option<i32> = sqlx::query_scalar(
    ...
    let max_version: Option<i32> = sqlx::query_scalar(
        "SELECT MAX(version) FROM vault_snapshots WHERE user_id = $1",
    )
    .bind(auth_user.user_id)
    .fetch_one(&mut *tx)
    .await?;
    ...
    let kek_bytes = apply_wrap_update(&mut tx, auth_user.user_id, "kek_wrap", &req.kek_wrap).await?;
    let recovery_bytes =
        apply_wrap_update(&mut tx, auth_user.user_id, "recovery_wrap", &req.recovery_wrap).await?;
